...
def train_function(model, train_dl, optimizer, loss, ep, epoch, train_loss_record, sanity_check=False, lr_search=False):
    ...
    for i, (input_train, target_train) in enumerate(train_dl):
        input_train = input_train.to("cuda")
        target_train = target_train.to("cuda")
        # forward calculation
...
def vali_function(model, model_save_dir, vali_dl, loss, ep, epoch, vali_loss_record, sanity_check=False,
    ...
    with torch.no_grad():
        for j, (input_eval, target_eval) in enumerate(vali_dl):
            ...
            target_eval = target_eval.to('cuda')
    ...
    return vali_loss_record

def test_function(model, vali_dl, loss):
    total_batch_eval = len(vali_dl)
    model.eval()
    vali_loss_sum = 0
    with torch.no_grad():
        for j, (input_eval, target_eval) in enumerate(vali_dl):
            ...
            vali_loss_sum = vali_loss_sum + loss_eval
        average_loss = vali_loss_sum / total_batch_eval
        print('test loss of the model: {:.6f}'.format(average_loss))
    return average_loss
